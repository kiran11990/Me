				...
				using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        applications.Add(new ApplicationData()
                        {
                            ...
                            ServiceLine = reader.IsDBNull(3) ? string.Empty : reader.GetString(3),
                            Application = reader.IsDBNull(4) ? string.Empty : reader.GetString(4),
                            ServiceClass = reader.IsDBNull(5) ? default(int) : reader.GetInt32(5),
                            RunOrGrow = reader.IsDBNull(6) ? default(int) : reader.GetInt32(6),
                            ApplicationGroup = reader.IsDBNull(7) ? string.Empty : reader.GetString(7),
                            StartDate = reader.GetDateTime(8),
                            EndDate = reader.GetDateTime(9),
                            EndToEnd = reader.GetBoolean(10),
                            Epm = reader.GetBoolean(11),
                            TM = reader.IsDBNull(12) ? string.Empty : reader.GetString(12),
                            ManagedCapacity = reader.IsDBNull(13) ? string.Empty : reader.GetString(13),
                            ManagedService = reader.IsDBNull(14) ? string.Empty : reader.GetString(14),
                            SoftwareAssetSearchableId = reader.GetString(15),
                            Remarks = reader.IsDBNull(16) ? string.Empty : reader.GetString(16),
                            OwnerAlias = reader.IsDBNull(17) ? string.Empty : reader.GetString(17),
